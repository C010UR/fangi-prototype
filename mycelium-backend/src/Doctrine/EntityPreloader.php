...
class EntityPreloader
{
    ...
    public function preload(
        ...
        if (null === $sourceEntitiesCommonAncestor) {
            return [];
        }
    ...
    private function preloadToMany(
        ...
        if (null === $sourceIdentifierAccessor || null === $sourcePropertyAccessor || null === $targetIdentifierAccessor) {
            throw new LogicException('Doctrine should use RuntimeReflectionService which never returns null.');
        }
    ...
    private function preloadOneToManyInner(
        ...
        if (null === $targetPropertyAccessor) {
            throw new LogicException('Doctrine should use RuntimeReflectionService which never returns null.');
        }
    ...
    private function preloadManyToManyInner(
        ...
        foreach ($manyToManyRows as $manyToManyRow) {
            ...
            if (false !== $targetEntity && !$this->entityManager->isUninitializedObject($targetEntity)) {
                $targetEntities[$targetEntityKey] = $targetEntity;
                continue;
            }
    ...
    private function preloadToOne(
        ...
        if (null === $sourcePropertyAccessor) {
            throw new LogicException('Doctrine should use RuntimeReflectionService which never returns null.');
        }
    ...
    private function addFetchJoinsToPreventFetchDuringHydration(
        ...
        foreach ($sourceClassMetadata->getAssociationMappings() as $fieldName => $associationMapping) {
            ...
            $isToOneInversed = $isToOne && false === $associationMapping['isOwningSide'];
            $isToOneAbstract = $isToOne && true === $associationMapping['isOwningSide'] && \count($targetClassMetadata->subClasses) > 0;
     ...
     * @param ClassMetadata<object> $classMetadata
     */
    private function getSingleIdPropertyAccessor(ClassMetadata $classMetadata): ?PropertyAccessor
    {
        $identifier = $classMetadata->getIdentifier();

        if (\count($identifier) > 1) {
            throw new InvalidArgumentException('Can\'t preload entity with multiple primary keys');
        }

        return $classMetadata->getPropertyAccessor(reset($identifier));
    }
